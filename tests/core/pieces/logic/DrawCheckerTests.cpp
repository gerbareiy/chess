...
namespace ServerTests
{
    ...
    {
    public:
        static std::shared_ptr<Chess::King> MakeKing(Chess::ePieceColor color, Chess::Coordinate coordinate)
        {
            return std::make_shared<Chess::King>(color, coordinate, false);
        }

        static std::shared_ptr<Chess::Chessboard> MakeBoard(
        ...
        static bool IsDraw(std::vector<std::shared_ptr<Chess::Piece>> pieces, Chess::ePieceColor toMove = Chess::ePieceColor::WHITE)
        {
    ...
    TEST(DrawCheckerTests, LoneKingsAreInsufficientMaterial)
    {
        EXPECT_TRUE(
            DrawCheckerTestHelpers::IsDraw(
                { DrawCheckerTestHelpers::MakeKing(Chess::ePieceColor::WHITE, { .file = 'E', .rank = 1 }),
                  DrawCheckerTestHelpers::MakeKing(Chess::ePieceColor::BLACK, { .file = 'E', .rank = 8 }) }));
    }
    ...
    TEST(DrawCheckerTests, StalemateIsDraw)
    {
        EXPECT_TRUE(
            DrawCheckerTestHelpers::IsDraw(
                { DrawCheckerTestHelpers::MakeKing(Chess::ePieceColor::WHITE, { .file = 'F', .rank = 7 }),
                  DrawCheckerTestHelpers::MakeKing(Chess::ePieceColor::BLACK, { .file = 'H', .rank = 8 }),
